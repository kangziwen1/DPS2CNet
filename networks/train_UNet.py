...
from networks.common.metrics_backup import Metrics
...
def parse_args():
    ...
    parser.add_argument(
        '--cfg',
        dest='config_file',
        default='SSC_configs/examples/BEV_Unet.yaml',
        metavar='FILE',
...
def fast_hist(pred, label, n):  # n==19
    ...
    bin_count = np.bincount(
        ...
        minlength=n**2)  # 返回每个点的 gt-label**19+pred—label的bincount
...
def train(model, optimizer, scheduler, dataset, _cfg, start_epoch, logger, tbwriter):
    ...
    for epoch in range(start_epoch, nbr_epochs + 1):
        ...
        for t, (data, indices) in enumerate(dset):
            ...
            # predict_labels = torch.argmax(scores[1], dim=1)  # B, L，W, H   or BN, 1
            # predict_labels = predict_labels.cpu().detach().numpy()
            # train_hist_list.append(fast_hist_crop(predict_labels, data['processed_label'].cpu().numpy(), unique_label))
            ...
            if (t + 1) % _cfg._dict['TRAIN']['SUMMARY_PERIOD'] == 0:
                ...
                # seg_iou = per_class_iu(sum(train_hist_list))
                # seg_miou = np.nanmean(seg_iou) * 100
                # loss_print += 'seg_miou = {:.6f},  '.format(seg_miou)
...
def validate(model, dset, _cfg, epoch, logger, tbwriter, metrics):
    ...
    with torch.no_grad():
        ...
        for t, (data, indices) in enumerate(dset):
            ...
            # predict_labels = torch.argmax(scores[1], dim=1)  # B, L，W, H   or BN, 1
            # predict_labels = predict_labels.cpu().detach().numpy()
            # val_hist_list.append(fast_hist_crop(predict_labels, data['processed_label'].cpu().numpy(), unique_label))
            ...
            if (t + 1) % _cfg._dict['VAL']['SUMMARY_PERIOD'] == 0:
                ...
                for key in loss.keys():
                    loss_print += '{} = {:.6f},  '.format(key, loss[key])
                # seg_iou = per_class_iu(sum(val_hist_list))
                # seg_miou = np.nanmean(seg_iou) * 100
                # loss_print += 'seg_miou = {:.6f},  '.format(seg_miou)
                logger.info(loss_print[:-3])
