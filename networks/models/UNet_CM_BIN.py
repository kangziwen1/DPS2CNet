...
class BEV_UNet_CM(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding,
        ...
        super(BEV_UNet_CM, self).__init__()
    ...
    def forward(self, x, x_ds1, x_ds2, x_ds3):
        ...
        x = self.up1(x5, x4_cat)  # 2 512 32 32
        x = self.up2(x, x3_cat)  # 2 256 64 64
        x = self.up3(x, x2_cat)  # 2 128 128 128
        x = self.up4(x, x1)  # 2 128 256 256
...
class up(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        if self.use_dropblock and epoch > 777:
            x = self.dropblock(x)
        return x
