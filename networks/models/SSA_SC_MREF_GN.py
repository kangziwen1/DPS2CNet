...
from mmseg.models import TverskyLoss

from networks.models.Conv_UNet_MREF_GN import BEV_UNet_Convnext_GR
from networks.models.UNet_CM_MREF_DCNv2 import BEV_UNet_DCNv2
from networks.models.UNet_MREF import BEV_UNet
...
class SSA_SC_MREF_GN(nn.Module):
    def __init__(self, class_num, input_dimensions, class_frequencies, pt_model='pointnet', fea_dim=3, pt_pooling='max',
        ...
        self.SSCNet = BEV_UNet_Convnext_GR(self.nbr_classes * self.n_height, self.n_height, self.dilation, self.bilinear,
                                     self.group_conv,
        ...
        if pt_model == 'pointnet':
            self.PPmodel = nn.Sequential(
                nn.GroupNorm(num_channels=fea_dim,num_groups=fea_dim),
    ...
    def compute_loss(self, scores, data):
        ...
        loss_fun = nn.CrossEntropyLoss(ignore_index=255, reduction='mean').to(device=device)
        tv_loss = TverskyLoss(ignore_index=255).to(device=device)

        loss_1_1_1 = criterion(scores['pred_semantic_1_1'], data['3D_LABEL']['1_1'].long())
        loss_1_1_2 = tv_loss(scores['pred_semantic_1_1'],
                                    data['3D_LABEL']['1_1'].long())
        # loss_1_1_3 = loss_fun(scores['pred_semantic_1_1'], data['3D_LABEL']['1_1'].long())

        # loss_seg = lovasz_softmax(torch.nn.functional.softmax(out_seg_data), data['processed_label'].long(),
        #                                       ignore=0) + loss_fun(out_seg_data, data['processed_label'].long())
        complet_loss = loss_1_1_1 + loss_1_1_2
        if self.stat == "train":
            seg_loss = loss_fun(out_seg_data, data['processed_label'].long()) + lovasz_softmax(
                torch.nn.functional.softmax(out_seg_data, dim=1),
                data['processed_label'].long(), ignore=255)
            loss_total = complet_loss + seg_loss
        else:
            seg_loss = complet_loss
            loss_total = seg_loss
        loss = {'total': loss_total, 'semantic_1_1': complet_loss, 'semantic_seg': seg_loss}
