...
from CrossModalCompletionConv import CrossModalFusion
...
class BEV_UNet_DCNv2(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding,
                 dropblock,is_cross_model=True):
        super(BEV_UNet_DCNv2, self).__init__()
        self.is_cross_model = is_cross_model
        ...
        self.down3 = down_dcnv2(512, 512, dilation, group_conv, circular_padding)
        self.down4 = down_dcnv2(1024, 512, dilation, group_conv, circular_padding)
        ...
        self.up4 = up(192, 128, 128, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout, cbam_num=4)
        self.dropout = nn.Dropout(p=0. if dropblock else dropout)
        self.outc = outconv(128, n_class)
        if self.is_cross_model:
            self.cross_model = CrossModalFusion(512)

    def forward(self, x, x_ds1, x_ds2, x_ds3, im_encoder=None):
...
class double_conv_dcnv2(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch, group_conv, dilation=1):
        super(double_conv_dcnv2, self).__init__()
        if group_conv:
            self.conv = nn.Sequential(
                DCNv2(in_ch, out_ch, 3, padding=1, groups=min(out_ch, in_ch)),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True),
                DCNv2(out_ch, out_ch, 3, padding=1, groups=out_ch),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
        else:
            self.conv = nn.Sequential(
                DCNv2(in_ch, out_ch, 3, padding=1),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True),
                DCNv2(out_ch, out_ch, 3, padding=1),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
...
class double_conv_circular_dcnv2(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch, group_conv, dilation=1):
        super(double_conv_circular_dcnv2, self).__init__()
        if group_conv:
            self.conv1 = nn.Sequential(
                DCNv2(in_ch, out_ch, 3, padding=(1, 0), groups=min(out_ch, in_ch)),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
            self.conv2 = nn.Sequential(
                DCNv2(out_ch, out_ch, 3, padding=(1, 0), groups=out_ch),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
        else:
            self.conv1 = nn.Sequential(
                DCNv2(in_ch, out_ch, 3, padding=(1, 0)),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
            self.conv2 = nn.Sequential(
                DCNv2(out_ch, out_ch, 3, padding=(1, 0)),
                nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True)
            )
...
class down_dcnv2(nn.Module):
    def __init__(self, in_ch, out_ch, dilation, group_conv, circular_padding):
        super(down_dcnv2, self).__init__()
        if circular_padding:
            self.mpconv = nn.Sequential(
                nn.MaxPool2d(2),
                double_conv_circular_dcnv2(in_ch, out_ch, group_conv=group_conv, dilation=dilation)
            )
        else:
            self.mpconv = nn.Sequential(
                nn.MaxPool2d(2),
                double_conv_dcnv2(in_ch, out_ch, group_conv=group_conv, dilation=dilation)
            )
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, up_ch, circular_padding, pix=False, bilinear=False, group_conv=False,
        ...
        self.act = nn.Sequential(nn.BatchNorm2d(up_ch), nn.SiLU(True))
