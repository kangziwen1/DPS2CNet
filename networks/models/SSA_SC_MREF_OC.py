...
from networks.models.UNet_OnlyEncoder import Asymm_3d_spconv
...
# Only Encoder
class SSA_SC_MREF_OC(nn.Module):
    def __init__(self, class_num, input_dimensions, class_frequencies, pt_model='pointnet', fea_dim=3, pt_pooling='max',
        ...
        self.SSCNet = BEV_UNet_Convnext_GR(self.nbr_classes * self.n_height, self.n_height, self.dilation,
                                           self.bilinear,
                                           self.group_conv,
    ...
    def compute_loss(self, scores, data):
        ...
        # loss_fun = FocalLoss(class_weight=list(class_weights)).to(device=device)
        ...
        loss_total = loss_1_1_1
        loss = {'total': loss_total, 'semantic_1_1': loss_total, 'semantic_seg': loss_total}
        #  loss = {'total': loss_total, 'semantic_1_1': complet_loss, 'semantic_seg': seg_loss,'weight_ce_loss':loss_1_1_1,'lovasz_loss':loss_1_1_2}
        return loss
