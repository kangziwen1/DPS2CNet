...
from networks.models.convnext_for_completion_v2Block import ConvNeXt_FC_V2, set_block_epoch_num, Block
...
class down_convnext:
    def __init__(self, model, count):
        self.model = model
        self.index = count - 1
        self.norm = BatchInstanceNorm2d(512).cuda()
...
class BEV_UNet_Convnext_GR(nn.Module):  # 2023 2 10 doubleconv --> convnext block
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding,
                 dropblock, pix, carafe):
        super(BEV_UNet_Convnext_GR, self).__init__()
        self.inc = inconv(64, 64, dilation, input_batch_norm, circular_padding)
        self.epoch = 0
        self.encoder = ConvNeXt_FC_V2(depths=[1, 1, 1, 1], dims=[128, 256, 512, 512], in_chans=64,
                                      drop_path_rate=0.277).cuda()
        self.down1 = down_convnext(self.encoder, 1)
        self.down2 = down_convnext(self.encoder, 2)
        self.down3 = down_convnext(self.encoder, 3)
        self.down4 = down_convnext(self.encoder, 4)

        self.up1 = up(1536, 512, 512, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout, cbam_num=0, pix=pix, carafe=carafe)
        self.up2 = up(1024, 256, 512, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout, cbam_num=0, pix=pix, carafe=carafe)  # 1
        self.up3 = up(512, 128, 256, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout, cbam_num=0, pix=pix, carafe=carafe)  # 2
        self.up4 = up(192, 128, 128, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout, cbam_num=0, pix=pix, carafe=carafe)  # 3
...
class convnext_block_bin(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch, group_conv, dilation=1):
        super(convnext_block_bin, self).__init__()
        self.stem = nn.Linear(in_ch, out_ch)
        self.conv = nn.Sequential(
            BatchInstanceNorm2d(out_ch),
            Block(out_ch),
        )
    ...
    def __init__(self, in_ch, out_ch, dilation, input_batch_norm, circular_padding):
        ...
        if input_batch_norm:
            if circular_padding:
                ...
                self.conv = nn.Sequential(
                    BatchInstanceNorm2d(in_ch),
                    convnext_block_bin(in_ch, out_ch, group_conv=False, dilation=dilation)
                )
        else:
            if circular_padding:
                self.conv = double_conv_circular(in_ch, out_ch, group_conv=False, dilation=dilation)
            else:
                self.conv = convnext_block_bin(in_ch, out_ch, group_conv=False, dilation=dilation)
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, up_ch, circular_padding, pix=False, bilinear=False, group_conv=False,
                 use_dropblock=False,
                 drop_p=0.277, cbam_num=1, carafe=False
                 ):
        ...
        if bilinear:
            ...
            self.up = nn.Conv2d(up_ch, 4 * up_ch, 1)
        elif carafe:
            from mmcv.ops.carafe import CARAFEPack
            self.up = CARAFEPack(up_ch, 2)
        else:
        ...
        if circular_padding:
            ...
            self.conv = convnext_block_bin(in_ch, out_ch, group_conv=group_conv)
        ...
        self.act = nn.Sequential(BatchInstanceNorm2d(up_ch), nn.GELU())
    ...
    def forward(self, x1, x2):
        if self.pix:
            x1 = torch.pixel_shuffle(self.up(x1), 2)
        else:
            x1 = self.up(x1)
        x1 = self.act(x1)
        # input is CHW
        ...
        if self.use_dropblock and epoch > 7:
            x = self.dropblock(x)
        return x
