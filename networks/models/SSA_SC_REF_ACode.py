...
from networks.models.UNet_REF import BEV_UNet
...
class SSA_SC_REF_AC(nn.Module):
    def __init__(self, class_num, input_dimensions, class_frequencies, pt_model='pointnet', fea_dim=3, pt_pooling='max',
        ...
        self.attention = nn.MultiheadAttention(7, 8, batch_first=True)
        self.SSCNet = BEV_UNet(self.nbr_classes * self.n_height, self.n_height, self.dilation, self.bilinear,
                               self.group_conv,
    ...
    def compute_loss(self, scores, data):
        ...
        loss_1_1 += lovasz_softmax(torch.nn.functional.softmax(scores['pred_semantic_1_1'], dim=1),
        ...
        loss_total = loss_1_1
        loss = {'total': loss_total, 'semantic_1_1': loss_1_1, 'semantic_seg': loss_1_1}
