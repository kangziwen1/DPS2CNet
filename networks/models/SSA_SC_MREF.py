...
from networks.models.Conv_UNet_MREF_GR import BEV_UNet_Convnext_GR
...
class SSA_SC_MREF(nn.Module):
    ...
    def compute_loss(self, scores, data):
        ...
        # seg_labelweights, compl_labelweights = self.get_class_weights()
        # seg_labelweights = seg_labelweights.to(device=target.device, dtype=target.dtype)
        # compl_labelweights = compl_labelweights.to(device=target.device, dtype=target.dtype)

        # criterion = nn.CrossEntropyLoss(weight=compl_labelweights, ignore_index=255, reduction='mean').to(device=device)
        # loss_fun = nn.CrossEntropyLoss(weight=seg_labelweights, ignore_index=255, reduction='mean').to(device=device)

        criterion = nn.CrossEntropyLoss(weight=None, ignore_index=255, reduction='mean').to(device=device)
        # loss_fun = nn.CrossEntropyLoss(weight=None, ignore_index=255, reduction='mean').to(device=device)
        ...
        if self.stat == "train":
            ...
            # loss_total = complet_loss + loss_1_1_3 + loss_1_1_2 + seg_loss
            ...
            seg_loss = complet_loss
            loss_total = complet_loss + loss_1_1_3 + loss_1_1_2
        # loss = {'total': loss_total, 'semantic_1_1': complet_loss, 'semantic_seg': seg_loss,'weight_ce_loss':loss_1_1_1,'geo_scal_loss':loss_1_1_3,
        #         'sem_scal_loss':loss_1_1_4}
