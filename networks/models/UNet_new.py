...
class BEV_UNet(nn.Module):
    ...
    def forward(self, x, x_ds1, x_ds2, x_ds3):
        ...
        x2_cat = torch.cat((x2, self.channel_reduction(x_ds1, x2.shape[1])), dim = 1)    # [B, 128, 128, 128] + [B, 128, 128, 128]    256
        x3 = self.down2(x2_cat)    # [B, 256, 64, 64]
        x3_cat = torch.cat((x3, self.channel_reduction(x_ds2, x3.shape[1])), dim = 1)    # [B, 256, 64, 64] + [B, 256, 64, 64]    512
