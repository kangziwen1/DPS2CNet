...
class BEV_UNet(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding,
        ...
        self.up3 = up(512, 128, 256, circular_padding, bilinear=bilinear, group_conv=group_conv,
                      use_dropblock=dropblock,
                      drop_p=dropout)
        self.up4 = up(192, 128,128, circular_padding, bilinear=bilinear, group_conv=group_conv, use_dropblock=dropblock,
                      drop_p=dropout)
    ...
    def forward(self, x, x_ds1, x_ds2, x_ds3):
        ...
        x = self.up1(x5, x4_cat)  # torch.Size([2, 512, 32, 32])
        x = self.up2(x, x3_cat)  # torch.Size([2, 256, 64, 64])
        x = self.up3(x, x2_cat)  # torch.Size([2, 128, 128, 128])
        x = self.up4(x, x1)  # torch.Size([2, 128, 256, 256])
        x = self.outc(self.dropout(x))  # torch.Size([2, 128, 256, 256])
        return x
...
#     def __init__(self, channel=16, reduction=16, kernel_size=7):
#         super(REF_Block, self).__init__()
#         self.upsample = nn.PixelShuffle(2)
#         self.attention_block = CBAMBlock(channel=channel, reduction=16, kernel_size=kernel_size)
#
#     def forward(self, x):
#         x = self.upsample(x)
...
class double_conv_gate(nn.Module):
    '''(conv => BN => ReLU) * 2'''

    def __init__(self, in_ch, out_ch, group_conv, dilation=1):
        super(double_conv_gate, self).__init__()
        if group_conv:
            self.conv = nn.Sequential(
                nn.Conv2d(in_ch, out_ch, 3, padding=1, groups=min(out_ch, in_ch)),
                nn.BatchNorm2d(out_ch),
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, up_ch, circular_padding, pix=False, bilinear=False, group_conv=False,
        ...
        self.attention_gate_block_1 = CBAMBlock(out_ch)
        self.attention_gate_block_2 = CBAMBlock(out_ch)
        self.attention_gate_block_3 = CBAMBlock(out_ch)
    ...
    def forward(self, x1, x2):
        ...
        x = self.conv(x)
        x = self.attention_gate_block_1(x)
        x = self.attention_gate_block_2(x)
        x = self.attention_gate_block_3(x)
