...
class SelfAttention(nn.Module):
    def __init__(self, in_channels):
        ...
        self.attention = nn.Sequential(CBAMBlock(in_channels), nn.GroupNorm(32,in_channels))
...
class BEV_UNet_Convnext_GR(nn.Module):
    def __init__(self, n_class, n_height, dilation, bilinear, group_conv, input_batch_norm, dropout, circular_padding,
        ...
        self.down1 = down_convnext(self.encoder, 1, nn.GroupNorm(32,128).to(device))
        self.down2 = down_convnext(self.encoder, 2, nn.GroupNorm(32,256).to(device))
        self.down3 = down_convnext(self.encoder, 3, nn.GroupNorm(32,512).to(device))
        self.down4 = down_convnext(self.encoder, 4, nn.GroupNorm(32,512).to(device))
...
class up(nn.Module):
    def __init__(self, in_ch, out_ch, up_ch, circular_padding, pix=False, bilinear=False, group_conv=False,
        ...
        self.act = nn.Sequential(nn.GroupNorm(32,up_ch), nn.SiLU(True))
